# usage
# python seg_chng_background.py <inp_img> <background_img> <output_image>
...
inputbgPath = sys.argv[2]
outputFilePath = sys.argv[3]


if inputFilePath is None or inputbgPath is None or outputFilePath is None:
  print("Bad parameters. Please specify input file path and output file path")
  exit()

modelType = "mobile_net_model"
if len(sys.argv) > 4 and sys.argv[4] == "accurate":
...
def run_visualization(filepath):
  ...
  background = Image.open(inputbgPath)

  overlay = drawSegment(resized_im, seg_map)
  
  background.paste(overlay, (150, 350), overlay)
  background.save(outputFilePath)
